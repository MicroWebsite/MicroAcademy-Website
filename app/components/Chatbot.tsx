...
export default function Chatbot() {
  ...
  const generateBotResponse = (userMessage: string): string => {
    ...
    if (lower.includes('fresher') || lower.includes('drive') || lower.includes('graduate'))
    ...
    if (lower.includes('career') || lower.includes('job') || lower.includes('opening') || lower.includes('hire'))
  ...
  return (
    <>
      {/* ── Chat Window ── */}
      <div
        className={`fixed bottom-24 right-10 z-[9999] w-[370px] max-w-[calc(100vw-2.5rem)] transition-all duration-300 ease-out origin-bottom-right ${isOpen
        ...
        <div className="flex flex-col rounded-2xl overflow-hidden shadow-2xl border border-border bg-white h-[520px] max-h-[70vh]">
          ...
          <div className="relative flex items-center gap-3 px-5 py-4 bg-linear-to-r from-primary to-primary-light">
            ...
            <div className="relative z-10 flex items-center gap-3 flex-1">
              ...
              <div>
                <h3 className="text-white font-bold text-sm tracking-wide">MicroBot</h3>
          ...
          {/* ── Messages Area ── */}
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-bg-cream-alt scroll-smooth" style={{
            scrollbarWidth: 'thin',
          ...
          {messages.length <= 1 && !isTyping && (
            <div className="px-4 py-3 bg-white border-t border-border-light">
              <p className="text-xs text-text-subtle font-semibold mb-2 uppercase tracking-wider">Quick Actions</p>
      ...
      <button
        ...
        className={`fixed bottom-5 right-5 z-[9999] w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 hover:scale-105 group ${isOpen
