...
const JobCardSection: React.FC<JobCardSectionProps> = ({
  ...
  return (
    <div className="w-full">
      <div className="hidden lg:block w-full rounded-2xl border border-black/5 shadow-[0_4px_24px_rgba(0,0,0,0.02)] relative overflow-hidden">
        <div className="absolute top-0 left-0 w-1 h-full bg-linear-to-b from-primary to-secondary rounded-l-2xl" />
        <div className="w-full overflow-x-auto pl-1">
          <table className="w-full min-w-[800px] border-collapse bg-white text-left font-sans">
            <thead>
              <tr className="bg-bg-cream border-b border-black/5 text-text-dark font-bold text-xs uppercase tracking-wider">
                <th className="py-5 px-6">Job ID</th>
                <th className="py-5 px-6">Job Title</th>
                <th className="py-5 px-6">Location</th>
                <th className="py-5 px-6">Experience</th>
                <th className="py-5 px-6">Education</th>
                {isContract && <th className="py-5 px-6">Contract Period</th>}
            ...
            <tbody className="divide-y divide-black/5">
              {jobs.map((pos, idx) => (
                ...
                >
                  ...
                  <td className="py-5 px-6 text-sm text-text-muted-alt align-middle">
                    {pos.location || "Not Specified"}
                  </td>
                  <td className="py-5 px-6 text-sm text-text-muted-alt align-middle">
                    {pos.experience
                      ? stripHtml(pos.experience)
                      : "Not Specified"}
                  </td>
                  <td className="py-5 px-6 text-sm text-text-muted-alt align-middle max-w-xs truncate">
                    {pos.education ? stripHtml(pos.education) : "Not Specified"}
                  </td>
                  {isContract && (
                    <td className="py-5 px-6 text-sm font-semibold text-primary align-middle">
                      {pos.contractPeriod || "Not Specified"}
                    </td>
                  )}
                  <td className="py-5 px-6 text-center align-middle">
                    <button
                      onClick={() => onApply(pos)}
                      className="px-5 py-2.5 rounded-full bg-linear-to-r from-btn-grad-start to-btn-grad-end text-white text-xs font-bold shadow-sm hover:shadow-md hover:brightness-110 active:scale-[0.97] transition-all duration-200 border-none font-sans cursor-pointer"
                    >
                      Apply Now
                    </button>
                  </td>
      ...
      <motion.div
        className="lg:hidden flex flex-wrap justify-center gap-6"
        ...
        {jobs.map((pos, idx) => (
          ...
          >
            ...
            <div className="flex flex-col flex-1 p-6 pl-7">
              <div className="mb-4">
                <span className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest text-primary bg-primary-50 px-3 py-1.5 rounded-full ring-1 ring-primary/10 font-sans">
                  <svg
                    className="w-3 h-3"
                    fill="none"
                    viewBox="0 0 24 24"
              ...
              <div className="flex flex-col gap-2.5 mb-6 flex-1">
                ...
                {pos.experience && stripHtml(pos.experience).trim() !== "" && (
                  <div className="flex items-start gap-2.5 text-sm text-text-muted-alt">
                    <svg
                      className="w-4 h-4 mt-0.5 shrink-0 text-text-subtle"
                      fill="none"
                    ...
                    <span>
                      <strong>Experience:</strong> {stripHtml(pos.experience)}
                    </span>
                  </div>
                )}
                {pos.education && stripHtml(pos.education).trim() !== "" && (
                  <div className="flex items-start gap-2.5 text-sm text-text-muted-alt font-sans">
                    <svg
                      className="w-4 h-4 mt-0.5 shrink-0 text-text-subtle"
                      fill="none"
                      viewBox="0 0 24 24"
                    ...
                    <span className="line-clamp-2">
                      <strong>Education:</strong> {stripHtml(pos.education)}
                    </span>
                  </div>
                )}
                {isContract && (
                  <div className="flex items-start gap-2.5 text-sm text-text-muted-alt font-sans">
                    <svg
                      className="w-4.5 h-4.5 mt-0.5 shrink-0 text-primary"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={1.8}
